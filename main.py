...
NODES = ["65.108.201.144", "65.108.201.140"]
...
def send_tx():
    ...
    if request.method == 'POST':
        if request.form:
            ...
            flash("<p>amount: {} PDN <br> fee: {} leaf <br> to: {}</p>".format(amount, fee, recipient))
...
def get_balance(address, t):
    try:
        r = requests.get("http://node.ffmpool.com:3000/ledger?wallet={}".format(address), timeout=t)
        return r.text
    except Exception as e:
        print("{} : {}".format(type(e), e))
        return 0
...
if __name__ == '__main__':
    ...
    webview.create_window("Pandanite Wallet", "http://localhost:52323/", width=820, height=500, text_select=True, resizable=False)
    webview.start()
    sys.exit()
